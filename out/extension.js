...
function activate(context) {
    ...
    const explainCommand = vscode.commands.registerCommand('somali-python-teacher.explainCode', async () => {
        ...
        if (editor) {
            ...
            if (text) {
                const panel = vscode.window.createWebviewPanel('pythonSomaliTeacher', 'Python Somali Teacher', vscode.ViewColumn.Two, { enableScripts: true });
    ...
    const openPanelCommand = vscode.commands.registerCommand('somali-python-teacher.openPanel', () => {
        const panel = vscode.window.createWebviewPanel('pythonSomaliTeacher', 'Python Somali Teacher', vscode.ViewColumn.One, {});
...
async function explainCodeWithAI(code) {
    try {
        const response = await (0, node_fetch_1.default)(OPENROUTER_URL, {
            method: 'POST',
            headers: {
...
async function explainPythonError(errorMessage, codeLine) {
    try {
        const response = await (0, node_fetch_1.default)(OPENROUTER_URL, {
            method: 'POST',
            headers: {
...
function getWebviewContent(code, intro, aiExplanation = "") {
    return `<!DOCTYPE html>
<html lang="so">
<head>
    <meta charset="UTF-8">
